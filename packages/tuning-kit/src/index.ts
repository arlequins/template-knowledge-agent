import { compareCanonicalText, stableTextHash } from "./canonical-text";
import {
  DOCUMENT_QA_PATTERN_KINDS,
  PATTERN_LANGUAGES,
} from "./pattern-constants";
import { SENSITIVE_PATTERNS, validatePatternBatch } from "./pattern-validation";

export { compareCanonicalText } from "./canonical-text";
export {
  DOCUMENT_QA_PATTERN_KINDS,
  PATTERN_LANGUAGES,
} from "./pattern-constants";
export { validatePatternBatch } from "./pattern-validation";
...
export function assignDeterministicSplits<T extends { groupKey: string }>(
  ...
  const groups = [
    ...
    (left, right) =>
      stableTextHash(left) - stableTextHash(right) ||
